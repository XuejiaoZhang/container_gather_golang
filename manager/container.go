...
var enableLoadReader = flag.Bool("enable_load_reader", true, "Whether to enable cpu load reader")

//var enableLoadReader = flag.Bool("enable_load_reader", false, "Whether to enable cpu load reader")
var HousekeepingInterval = flag.Duration("housekeeping_interval", 5*time.Second, "Interval between container housekeepings")
...
func jitter(duration time.Duration, maxFactor float64) time.Duration {
	...
	wait := duration
	// duration < wait < duration+1*1*duration = 2*duration because 0.0 < rand.Float64() < 1.0 maxFactor=1.0
...
func (c *containerData) getPsOutput(inHostNamespace bool, format string) ([]byte, error) {
	...
	//glog.Infof("ps out 2 %t %s %d %s %s %s %v\n", inHostNamespace, command, len(args), args[0], args[2], args[2], c)
	if err != nil {
...
func newContainerData(containerName string, memoryCache *memory.InMemoryCache, handler container.ContainerHandler, logUsage bool, collectorManager collector.CollectorManager, maxHousekeepingInterval time.Duration, allowDynamicHousekeeping bool) (*containerData, error) {
	...
	//glog.Infof("***********enableLoadReader ************: %+v", enableLoadReader)
	//glog.Infof("***********loadAvg 2 ************: %+v", cont)
	err = cont.updateSpec()
	if err != nil {
		return nil, err
	}
	...
	if self.allowDynamicHousekeeping {
		...
		if err != nil {
			...
			if stats[0].StatsEq(stats[1]) && (self.housekeepingInterval < self.maxHousekeepingInterval) {
				...
				fmt.Println("housekeepingInterval 1", self.housekeepingInterval, self.maxHousekeepingInterval)
				...
				fmt.Println("housekeepingInterval 2", self.housekeepingInterval, *HousekeepingInterval)
				self.housekeepingInterval = *HousekeepingInterval
			}
		}
	}

	//fmt.Println(" ** lastHousekeeping  ***", self.allowDynamicHousekeeping, lastHousekeeping, lastHousekeeping.Add(jitter(self.housekeepingInterval, 1.0)))
	...
	*/
	...
	for {
		...
		default:
			...
			//fmt.Println("duration", c.info.Name, duration)
		}
		...
		//fmt.Println("time interval 22", c.info.Name, lastHousekeeping, next)
...
func (c *containerData) updateStats() error {
	...
	if customFlag == true {
		...
		customMetrics := make(map[string][]v1.MetricVal)
		netitem := paas.GetNet(c.info.Name)
		for k, v := range netitem {
			customMetrics[k] = []v1.MetricVal{
				{IntValue: v, Timestamp: currentTime},
			}
		}
		ipitem := paas.GetIP(c.info.Name)
		for k, v := range ipitem {
			customMetrics[k] = []v1.MetricVal{
				{StringValue: v, Timestamp: currentTime},
			}
		}
		stats.CustomMetrics = customMetrics
		//fmt.Println("CustomMetrics ", c.info.Name, stats.CustomMetrics)
	}
	//paas end
