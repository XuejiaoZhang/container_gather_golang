...
//transfer int to *time.Duration  unit: s
func TimeDurationTransfer(num int) *time.Duration {
	ret := time.Duration(num * 1000 * 1000 * 1000)
	//var ret time.Duration = num * time.Second
	var retInterval *time.Duration
	retInterval = &ret
	return retInterval
}

func TestCollectHousekeepingIntervalCheck(t *testing.T) {
	var test_data = [3]IntervalCheck{
		{TimeDurationTransfer(1), TimeDurationTransfer(2), TimeDurationTransfer(4)},
		{TimeDurationTransfer(4), TimeDurationTransfer(2), TimeDurationTransfer(4)},
		{TimeDurationTransfer(5), TimeDurationTransfer(2), TimeDurationTransfer(5)},
	}

	for _, v := range test_data {
		if *v.retInterval != *CollectHousekeepingIntervalCheck(v.desInterval, v.compareInterval) {
			t.Errorf("CollectHousekeepingIntervalCheck(%v, %v)'s return %v!= %v", *v.desInterval, *v.compareInterval, *CollectHousekeepingIntervalCheck(v.desInterval, v.compareInterval), *v.retInterval)
		}
	}
}

func TestLocalFileHousekeepingIntervalCheck(t *testing.T) {
	var test_data = [6]IntervalCheck{
		{TimeDurationTransfer(40), TimeDurationTransfer(60), TimeDurationTransfer(2 * 60)},
		{TimeDurationTransfer(40), TimeDurationTransfer(50), TimeDurationTransfer(2 * 60)},
		{TimeDurationTransfer(40), TimeDurationTransfer(70), TimeDurationTransfer(2 * 70)},
		{TimeDurationTransfer(140), TimeDurationTransfer(70), TimeDurationTransfer(2 * 70)},
		{TimeDurationTransfer(150), TimeDurationTransfer(70), TimeDurationTransfer(150)},
		{TimeDurationTransfer(130), TimeDurationTransfer(70), TimeDurationTransfer(2 * 70)},
	}

	for _, v := range test_data {
		if *v.retInterval != *LocalFileHousekeepingIntervalCheck(v.desInterval, v.compareInterval) {
			t.Errorf("LocalFileHousekeepingIntervalCheck(%v, %v)'s return %v!= %v", *v.desInterval, *v.compareInterval, *LocalFileHousekeepingIntervalCheck(v.desInterval, v.compareInterval), *v.retInterval)
		}
	}
}

func TestLocalFileDirCheck(t *testing.T) {
	filename := "/tmp/asdfghjkl1234567890poiuit/fortest"
	var localfile *string
	localfile = &filename
	var test_data = []LocalFileCheck{
		{localfile, true},
	}
	for _, v := range test_data {
		dir, _ := path.Split(*localfile)
		LocalFileDirCheck(v.localfile)
		if dir == "" || paas.Exist(dir) {
			os.Remove(dir)
		} else {
			t.Errorf("the dir %s of local file %s doesn't exist.", dir, v.localfile)
		}
	}
}
