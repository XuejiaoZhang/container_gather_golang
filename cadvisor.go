...
func main() {
	...
	*/
	...
	//paas start
	//scan local file
	go containerManager.LocalFileHousekeeping(containerManager)

	// Collect data from memory every 1 minute.
	go containerManager.CollectHousekeeping(containerManager)
	//if _, err := go containerManager.CollectHousekeeping(containerManager); err != nil {
	//	glog.Fatalf("Failed to collect data from memory : %v", err)
	//}
